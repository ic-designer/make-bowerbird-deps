...
def-one-dependency-entry:
	...
	$(call bowerbird::test::compare-string,$(ID.MK),$(abspath path/entry))
...
def-one-dependency-path:
	$(eval $(call bowerbird::deps::define-dependency-constants,ID,url,version,path,entry))
	$(call bowerbird::test::compare-string,$(ID/PATH),$(abspath path))
...
def-one-dependency-url:
	$(eval $(call bowerbird::deps::define-dependency-constants,ID,url,version,path,entry))
	$(call bowerbird::test::compare-string,$(ID/URL),url)
...
def-one-dependency-version:
	$(eval $(call bowerbird::deps::define-dependency-constants,ID,url,version,path,entry))
	$(call bowerbird::test::compare-string,$(ID/VERSION),version)
